...
class CampaignCard extends StatelessWidget {
  ...
  String _cleanCardDescription(String value) {
    ...
    final seen = <String>{};
    final unique = <String>[];
    for (final part in parts) { if (seen.add(part.toLowerCase().replaceAll(RegExp(r'\s+'), ' '))) unique.add(part); if (unique.join(' ').length >= 320) break; }
  ...
  String? _benefitFromText(String title, String description) {
    ...
    if (percent != null) {
      ...
      final contextEnd = (percent.end + 120).clamp(0, text.length);
      final context = text.substring(percent.end, contextEnd);
    ...
    // Parse the amount first, then inspect the text immediately after TL.
    // This avoids fragile regexes around Turkish apostrophes such as TL'ye/TL’ye.
    final tl = RegExp(r'(\d{1,3}(?:[.\s]\d{3})*|\d+)\s*TL', caseSensitive: false);
    for (final match in tl.allMatches(text).toList().reversed) {
      final afterEnd = (match.end + 90).clamp(0, text.length);
      final after = text.substring(match.end, afterEnd);
      final kind = RegExp(r'bonus|puan|chip.?para|indirim|promosyon', caseSensitive: false).firstMatch(after);
      if (kind != null) {
        final amount = match.group(1)!.replaceAll(RegExp(r'\s+'), '.');
        final word = kind.group(0)!.toLowerCase();
        if (word.contains('bonus') || word.contains('puan') || word.contains('chip')) return '$amount TL Bonus';
        if (word.contains('indirim')) return '$amount TL İndirim';
        return '$amount TL Promosyon';
      }
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Card(
      ...
      child: InkWell(onTap: onTap, child: Padding(padding: const EdgeInsets.all(14), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 8), Text(description.isEmpty ? 'Kampanya detaylarını görmek için dokun.' : description, maxLines: 3, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Color(0xFF6F687A), fontSize: 12.5, height: 1.35)),
        const SizedBox(height: 10), Wrap(spacing: 7, runSpacing: 6, children: [_Tag(Icons.local_offer_outlined, campaign.category), if (campaign.bankName.isNotEmpty) _Tag(Icons.account_balance_outlined, campaign.bankName), if (campaign.cardName.isNotEmpty) _Tag(Icons.credit_card_outlined, campaign.cardName)]),
        const SizedBox(height: 11), Row(children: [if (advantage != null) Expanded(child: _Advantage(advantage)) else const Spacer(), const SizedBox(width: 10), ElevatedButton.icon(onPressed: onTap, style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF6D3DF5), foregroundColor: Colors.white, elevation: 0, padding: const EdgeInsets.symmetric(horizontal: 15, vertical: 9), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(11))), icon: const Text('Detaylar', style: TextStyle(fontSize: 12.5, fontWeight: FontWeight.w800)), label: const Icon(Icons.arrow_forward, size: 16))])
      ]))),
    );
  }
}
