...
class _CampaignsPageState extends State<CampaignsPage> {
  ...
  // Günlük arama kelimelerini kampanya kategorilerine bağlar.
  // Örn: LPG -> Akaryakıt, hamburger -> Restoran.
  String detectCategory(String text) {
    final q = normalizeMatch(text);

    const map = {
      'E-ticaret': [
        'trendyol', 'hepsiburada', 'amazon', 'n11', 'online',
        'e-ticaret', 'eticaret', 'internet alışverişi',
        'internetten alışveriş', 'web alışveriş',
      ],
      'Akaryakıt': [
        'benzin', 'motorin', 'mazot', 'akaryakıt', 'akaryakit',
        'petrol', 'lpg', 'otogaz', 'yakıt', 'yakit',
        'benzinlik', 'akaryakıt istasyonu', 'akaryakit istasyonu',
        'opet', 'shell', 'bp', 'total', 'petrol ofisi',
      ],
      'Market': [
        'market', 'alışveriş', 'alisveris', 'erzak', 'gıda',
        'gida', 'migros', 'carrefour', 'a101', 'bim', 'şok', 'sok',
      ],
      'Restoran': [
        'restoran', 'restaurant', 'yemek', 'hamburger', 'burger',
        'pizza', 'döner', 'doner', 'kebap', 'kebab', 'lokanta',
        'kahve', 'cafe', 'kafe', 'fast food', 'yemeksepeti',
        'getir yemek',
      ],
      'Elektronik': [
        'telefon', 'cep telefonu', 'laptop', 'bilgisayar', 'tablet',
        'televizyon', 'elektronik', 'teknoloji', 'kulaklık', 'kulaklik',
      ],
      'Giyim': [
        'giyim', 'kıyafet', 'kiyafet', 'elbise', 'pantolon',
        'ayakkabı', 'ayakkabi', 'zara', 'defacto',
      ],
    };

    for (final entry in map.entries) {
      if (entry.value.any((word) => q.contains(normalizeMatch(word)))) {
        return entry.key;
      }
    }
  ...
  // Kampanya category alanı farklı yazılsa bile ilgili kampanyayı bulur.
  bool categoryMatches(String wantedCategory, String campaignText) {
    final q = normalizeMatch(campaignText);

    const synonyms = {
      'Akaryakıt': [
        'akaryakıt', 'akaryakit', 'benzin', 'motorin', 'mazot',
        'lpg', 'otogaz', 'yakıt', 'yakit', 'petrol', 'benzinlik',
        'opet', 'shell', 'bp', 'total', 'petrol ofisi',
      ],
      'Restoran': [
        'restoran', 'restaurant', 'yemek', 'hamburger', 'burger',
      ...
      'Market': [
        'market', 'alışveriş', 'alisveris', 'erzak', 'gıda', 'gida',
        'migros', 'carrefour', 'a101', 'bim', 'şok', 'sok',
      ],
      'E-ticaret': [
        'e-ticaret', 'eticaret', 'online', 'internet alışverişi',
        'internetten alışveriş', 'trendyol', 'hepsiburada', 'amazon', 'n11',
      ],
      'Elektronik': [
        'elektronik', 'teknoloji', 'telefon', 'laptop', 'bilgisayar',
        'tablet', 'televizyon',
      ],
      'Giyim': [
        'giyim', 'kıyafet', 'kiyafet', 'elbise', 'pantolon',
    ...
    final words = synonyms[wantedCategory];
    if (words == null) {
      return q.contains(normalizeMatch(wantedCategory));
    }

    return words.any((word) => q.contains(normalizeMatch(word)));
  }

  String normalizeMatch(String value) {
    return value
        .toLowerCase()
        .replaceAll('ı', 'i')
        .replaceAll('ş', 's')
  ...
  bool fieldMatches(String rule, String value) {
    final r = normalizeMatch(rule);
    final v = normalizeMatch(value);
    if (r.isEmpty || r == '*') return true;
    if (v.isEmpty) return false;
    return r == v || r.contains(v) || v.contains(r);
  }
...
class _MyCardsPageState extends State<MyCardsPage> {
  ...
  final cardMap = const {
    ...
    'Türkiye Finans': ['Happy Card'],
    'Albaraka Türk': ['Bonus Card'],
    'ING': ['ING Bonus'],
    'Fibabanka': ['Bonus Card'],
    'HSBC': ['HSBC Premier'],
    'Anadolubank': ['Anadolubank Kart'],
    'Odeabank': ['Odeabank Kart'],
    'Enpara': ['Enpara Kredi Kartı'],
    'CEPTETEB': ['CEPTETEB Bonus'],
    'Alternatif Bank': ['Maximum'],
  };
