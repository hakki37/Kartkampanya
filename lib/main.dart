...
class _CampaignsPageState extends State<CampaignsPage> {
  ...
  double rewardFor(Map<String, dynamic> campaign) {
    ...
    // Öncelik: ödül olduğu açıkça belli olan ifadeler.
    final explicitPatterns = <RegExp>[
      RegExp(r'(\d+(?:[.,]\d+)?)\s*tl\s*(?:ye|ya)?\s*varan'),
      RegExp(r'(\d+(?:[.,]\d+)?)\s*tl\s*(?:worldpuan|world\s*puan|puan|hediye|bonus|iade)'),
      RegExp(r'(?:toplamda|kazanc(?:ı|i)?|odul|ödül)\s*(\d+(?:[.,]\d+)?)\s*tl'),
    ];
    ...
    if (bestReward > 0) {
      if (max > 0 && bestReward > max) return max;
      return bestReward;
    }

    // Ödül ifadesi bulunamazsa genel TL taraması yap; ancak
    // minimum harcama/"ve üzeri" tutarlarını ödül kabul etme.
    final matches = RegExp(
      r'(\d+(?:[.,]\d+)?)\s*tl',
      caseSensitive: false,
    ).allMatches(text);
    ...
    for (final match in matches) {
      final value = parseMoney(match.group(1)!);
      if (value <= 0) continue;
      ...
      final after = text.substring(
        afterEnd,
        afterEnd + 30 > text.length ? text.length : afterEnd + 30,
      );
      ...
      final isMinimumSpend =
          ...
          before.contains('alisveristen') ||
          before.contains('alışveriş') ||
          after.contains('ve uzeri') ||
          after.contains('uzeri') ||
          after.contains('üzeri');
...
class SmartCampaignCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final campaignTitle = decodeHtmlEntities(
        '${campaign['title'] ?? ''}').trim();
