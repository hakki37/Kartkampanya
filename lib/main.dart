...
import 'wallpaper_extension.dart';
...
class _HomeShellState extends State<HomeShell> {
  ...
  @override void initState(){super.initState(); loadFavorites();}
  Future<void> loadFavorites() async { final p=await SharedPreferences.getInstance(); if(mounted)setState(()=>favorites.addAll(p.getStringList('favorites')??[])); }
  Future<void> toggleFavorite(String url) async { final p=await SharedPreferences.getInstance(); setState(()=>favorites.contains(url)?favorites.remove(url):favorites.add(url)); await p.setStringList('favorites',favorites.toList()); }
  @override Widget build(BuildContext context){
    final pages=[HomePage(favorites:favorites,onFavorite:toggleFavorite),const PrayerPage(),FavoritesPage(favorites:favorites,onFavorite:toggleFavorite)];
    return Scaffold(body:pages[index],bottomNavigationBar:NavigationBar(selectedIndex:index,onDestinationSelected:(v)=>setState(()=>index=v),destinations:const[
...
class HomePage extends StatelessWidget {
  ...
  @override Widget build(BuildContext context)=>CustomScrollView(slivers:[
    ...
    SliverToBoxAdapter(child:Padding(padding:const EdgeInsets.fromLTRB(16,0,16,18),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
      Text('Huzura bir adım daha…',style:Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight:FontWeight.w700)),const SizedBox(height:14),
      Container(padding:const EdgeInsets.all(18),decoration:BoxDecoration(borderRadius:BorderRadius.circular(24),gradient:const LinearGradient(begin:Alignment.topLeft,end:Alignment.bottomRight,colors:[Color(0xFF195C50),Color(0xFF2E8772)])),child:const Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
        Icon(Icons.format_quote,color:Colors.white70,size:30),SizedBox(height:6),Text('Ameller niyetlere göredir.',style:TextStyle(color:Colors.white,fontSize:22,fontWeight:FontWeight.bold)),SizedBox(height:8),Text('Buhârî, Bed’ü’l-vahy, 1',style:TextStyle(color:Colors.white70)),
      ])),
      const SizedBox(height:18),const Text('Günün Ayeti',style:TextStyle(fontSize:18,fontWeight:FontWeight.bold)),const SizedBox(height:8),
      ...
      const SizedBox(height:20),const Text('İslami Görseller',style:TextStyle(fontSize:18,fontWeight:FontWeight.bold)),const SizedBox(height:10),
    ]))),
    SliverPadding(padding:const EdgeInsets.symmetric(horizontal:16),sliver:SliverList(delegate:SliverChildBuilderDelegate((context,i){
      ...
      return Padding(padding:const EdgeInsets.only(bottom:16),child:ClipRRect(borderRadius:BorderRadius.circular(24),child:Stack(children:[
        ...
        Positioned(left:16,right:8,bottom:10,child:Row(children:[const Expanded(child:Text('Allah’ın zikriyle kalpler huzur bulur.',style:TextStyle(color:Colors.white,fontWeight:FontWeight.bold,fontSize:16))),IconButton(onPressed:()=>onFavorite(url),icon:Icon(fav?Icons.favorite:Icons.favorite_border,color:Colors.white)),IconButton(onPressed:()=>setWallpaper(context,url),icon:const Icon(Icons.wallpaper,color:Colors.white))])),
      ]));
...
Future<void> setWallpaper(BuildContext context,String url) async { try { final result=await WallpaperManagerFlutter().setWallpaperFromNetwork(url,WallpaperManagerFlutter.HOME_SCREEN); if(context.mounted)ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text(result?'Arka plan ayarlandı.':'Arka plan ayarlanamadı.'))); } catch(_){if(context.mounted)ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content:Text('Arka plan ayarlanamadı.')));}}
...
class _PrayerPageState extends State<PrayerPage> {
  String city='Istanbul'; Map<String,dynamic>? timings; bool loading=true;
  @override void initState(){super.initState();fetch();}
  Future<void> fetch() async { setState(()=>loading=true); try{final d=DateTime.now();final u=Uri.parse('https://api.aladhan.com/v1/timingsByCity/${d.day}-${d.month}-${d.year}?city=${Uri.encodeComponent(city)}&country=Turkey&method=13');final r=await http.get(u);final j=jsonDecode(r.body);if(j['code']==200)timings=Map<String,dynamic>.from(j['data']['timings']);}catch(_){}finally{if(mounted)setState(()=>loading=false);}}
  @override Widget build(BuildContext context){
    ...
    return Scaffold(appBar:AppBar(title:const Text('Namaz Vakitleri',style:TextStyle(fontWeight:FontWeight.bold)),actions:[IconButton(onPressed:fetch,icon:const Icon(Icons.refresh))]),body:ListView(padding:const EdgeInsets.all(16),children:[
      ...
      const SizedBox(height:16),if(loading)const Center(child:Padding(padding:EdgeInsets.all(30),child:CircularProgressIndicator()))else...names.entries.map((e)=>Card(child:ListTile(leading:CircleAvatar(child:Icon(e.key=='Maghrib'?Icons.nights_stay:Icons.access_time)),title:Text(e.value,style:const TextStyle(fontWeight:FontWeight.w700)),trailing:Text('${timings?[e.key]??'--:--'}',style:const TextStyle(fontSize:20,fontWeight:FontWeight.bold))))),
      const SizedBox(height:12),const Card(child:Padding(padding:EdgeInsets.all(16),child:Row(children:[Icon(Icons.volume_up_outlined),SizedBox(width:12),Expanded(child:Text('Ezan bildirimleri ve ezan sesi sonraki sürümde vakitlere otomatik bağlanacak.'))])))
    ]));
  }
}
