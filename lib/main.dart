...
class _CampaignsPageState extends State<CampaignsPage> {
  ...
  Future<List<Map<String, dynamic>>> fetchCampaigns() async {
    ...
    // Kampanyanın açıklama/ödül verisi campaigns tablosundan,
    // banka-kart kuralları campaign_rules tablosundan alınır.
    // Böylece kampanya metni ve kazanç bilgisi kaybolmaz.
    final results = await Future.wait([
      client.from('campaigns').select('*').order('id', ascending: false),
    ...
    String first(Map<String, dynamic> row, List<String> fields) {
      for (final field in fields) {
        final value = clean(row[field]);
    ...
    List<Map<String, dynamic>> rulesFor(
      Map<String, dynamic> campaign,
    ) {
      final matches = <Map<String, dynamic>>[];

      final campaignId = first(campaign, [
        'id',
        'campaign_id',
        'campaignId',
      ]);

      final campaignUrl = first(campaign, [
        'source_url',
        'sourceUrl',
        'url',
        'campaign_url',
      ...
      final campaignTitle = first(campaign, [
        'title',
        'campaign_title',
        'name',
      ]);

      final campaignMerchant = first(campaign, [
        'merchant',
        'merchant_name',
      ]);

      for (final rule in ruleRows) {
        // 1) Varsa açık campaign_id ilişkisini kullan.
        final ruleCampaignId = first(rule, [
          'campaign_id',
          'campaignId',
          'campaignid',
        ]);
        ...
        if (campaignId.isNotEmpty &&
            ruleCampaignId.isNotEmpty &&
            same(campaignId, ruleCampaignId)) {
          matches.add(rule);
          continue;
        }

        // 2) URL ile eşleştir.
        final ruleUrl = first(rule, [
          'source_url',
          'sourceUrl',
          'url',
          'campaign_url',
        ...
        if (campaignUrl.isNotEmpty &&
            ruleUrl.isNotEmpty &&
            same(campaignUrl, ruleUrl)) {
          matches.add(rule);
          continue;
        }

        // 3) Başlık + merchant ile eşleştir.
        final ruleTitle = first(rule, [
          'title',
          'campaign_title',
          'name',
        ]);
        final ruleMerchant = first(rule, [
          'merchant',
          'merchant_name',
        ]);

        final titleMatch = campaignTitle.isNotEmpty &&
            ruleTitle.isNotEmpty &&
            same(campaignTitle, ruleTitle);

        final merchantMatch = campaignMerchant.isNotEmpty &&
            ruleMerchant.isNotEmpty &&
            same(campaignMerchant, ruleMerchant);

        if (titleMatch || (merchantMatch && campaignTitle.isEmpty)) {
          matches.add(rule);
        }
      }
    ...
    for (final sourceCampaign in campaignRows) {
      final campaign = Map<String, dynamic>.from(sourceCampaign);

      // campaigns içindeki mevcut metin/ödül alanlarına DOKUNMA.
      campaign['title'] = campaign['title']?.toString() ?? '';
      campaign['source_url'] =
          campaign['source_url']?.toString() ?? '';
      campaign['merchant'] =
          campaign['merchant']?.toString() ?? '';
      campaign['campaign_text'] =
          campaign['campaign_text']?.toString() ?? '';
      campaign['conditions'] =
          campaign['conditions']?.toString() ?? '';
      campaign['terms'] =
          campaign['terms']?.toString() ?? '';

      final rules = rulesFor(campaign);

      // Kural varsa sadece kart eşleştirmesinde kullan.
      // Kampanyanın açıklaması, kazancı ve diğer gösterim alanları
      // campaigns tablosundaki orijinal değer olarak kalır.
      campaign['_rules'] = rules;

      list.add(campaign);
    }

    // campaigns tablosunda olmayan ama campaign_rules'da bulunan
    // kampanyaları da kaybetme. Böylece sadece tek bankanın değil,
    // diğer bankaların kuralları da uygulamada görünebilir.
    final representedRuleRows = <int>{};
      ...
      if (rules is List) {
        for (final rule in rules) {
          for (var i = 0; i < ruleRows.length; i++) {
            if (identical(ruleRows[i], rule)) {
              representedRuleRows.add(i);
            }
          }
        }
      }
    ...
    for (var i = 0; i < ruleRows.length; i++) {
      if (representedRuleRows.contains(i)) continue;

      final rule = Map<String, dynamic>.from(ruleRows[i]);

      // Yalnızca kural tablosunda bulunan bir kampanyaysa,
      // kendi verisini kampanya olarak ekle.
      final title = rule['title'] ?? rule['campaign_title'];
      final textValue = rule['campaign_text'];

      if (title == null && textValue == null) continue;

      final campaign = Map<String, dynamic>.from(rule);
      campaign['_rules'] = [rule];
      list.add(campaign);
    }
  ...
  bool _ruleMatches(String rule, String actual) {
    ...
    bool matchesOne(String item) {
      ...
      final xs = <String>{
        x,
        ...(aliases[x] ?? const <String>[]),
      };
      final ys = <String>{
        a,
        ...(aliases[a] ?? const <String>[]),
      };

      return xs.any((left) => ys.contains(left));
    }
  ...
  double? parseAmount(String text) {
    ...
    var raw = matches.last.group(1)!.replaceAll(' ', '');

    // Türkçe yazım: 3.000 TL / 3.000,50 TL
  ...
  bool _matchesSingleRule(
    ...
    // network veya eligible_networks kullanılabilir.
  ...
  bool cardMatches(
    ...
    // Bir kampanyanın birden fazla banka/kart kuralı olabilir.
    // Herhangi biri eşleşiyorsa kart uygundur.
    ...
    // Kural bulunamazsa kampanyanın kendi banka/kart alanlarını kullan.
  ...
  double rewardFor(Map<String, dynamic> campaign) {
    ...
    final text = _norm('$title $campaignText $conditions $terms');
    double bestReward = 0;

    // Önce açıkça toplam ödülü belirten ifadeleri yakala:
    // "1.500 TL'ye varan", "600 TL'ye varan" vb.
    ...
    for (final match in maxMatches) {
      ...
      if (value > bestReward) bestReward = value;
    }

    // Diğer TL tutarlarını kontrol et; minimum harcama tutarlarını
    // ödül olarak kabul etme.
    final matches = RegExp(
      r'(\d+(?:[.,]\d+)?)\s*tl',
      caseSensitive: false,
    ).allMatches(text);
    ...
    for (final match in matches) {
      ...
      final before = text.substring(
        match.start > 80 ? match.start - 80 : 0,
        match.start,
      );
      ...
      final after = text.substring(
        afterEnd,
        afterEnd + 15 > text.length ? text.length : afterEnd + 15,
      );
    ...
    if (max > 0 && bestReward > max) bestReward = max;
    return bestReward;
  }
