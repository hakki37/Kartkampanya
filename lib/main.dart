...
class _LoginPageState extends State<LoginPage> {
  ...
  List<String> fieldMatches(
    ...
    final aliases = <String, List<String>>{
      ...
      'kargo': ['kargo', 'kurye', 'teslimat'],
      'ulasim': ['ulasim', 'otobus', 'ucak', 'havayolu', 'taksi'],
      'akaryakit': ['akaryakit', 'benzin', 'motorin', 'dizel', 'lpg', 'otogaz'],
    };

    final terms = <String>{q, ...(aliases[q] ?? const <String>[])};

    return terms.where((term) => haystack.contains(term)).toList();
  }
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  const Text(
                    'Ödül, minimum harcama ve bitiş tarihini karşılaştır.',
                  ),
                  const SizedBox(height: 16),
                  ...selected.map((c) {
                    final merchant =
                        decodeHtmlEntities('${c['merchant'] ?? ''}');
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(
                              '💰 Avantaj: ${reward.toStringAsFixed(0)} TL',
                            ),
                            Text(
                              '🛒 Minimum: ${min > 0 ? '${min.toStringAsFixed(0)} TL' : 'Yok'}',
                            ),
                            Text(
                              left >= 0
