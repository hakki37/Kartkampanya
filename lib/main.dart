...
class _CampaignsPageState extends State<CampaignsPage> {
  ...
  Future<List<Map<String, dynamic>>> fetchCampaigns() async {
    // campaign_rules içindeki her satır kampanyanın bir kart/banka kuralı
    // olabilir. Kampanya metni, ödül ve açıklama kaybolmasın diye satırları
    // değiştirmeden alıyor; aynı kampanyanın birden fazla kart kuralını
    // daha sonra _rules altında birleştiriyoruz.
    final rows = await Supabase.instance.client
        .from('campaign_rules')
        .select()
    ...
    String keyOf(Map<String, dynamic> row) {
      ...
      final title = clean(row['campaign_title'] ?? row['title']);
      final textValue = clean(row['campaign_text']);
      final merchant = clean(row['merchant']);
      final source = clean(row['source_url']);

      // Önce kampanya başlığı; başlık yoksa kampanya metni; bunlar da
      // yoksa merchant + source kullanılır.
      return [title, textValue, merchant, source]
    ...
    for (final entries in grouped.values) {
      // En dolu açıklamayı temsilci olarak seç.
      entries.sort((a, b) =>
          '${b['campaign_text'] ?? b['campaign_title'] ?? ''}'.length
              .compareTo(
            '${a['campaign_text'] ?? a['campaign_title'] ?? ''}'.length,
          ));

      final campaign = Map<String, dynamic>.from(entries.first);
      campaign['_rules'] = entries
          .map((e) => Map<String, dynamic>.from(e))
          .toList(growable: false);

      // Aynı kampanyanın farklı banka kurallarında ödül/limit tekrar
      // ediliyorsa veriyi kaybetme. Pozitif değerlerin en yükseğini al.
      double maxPositive(String field) {
        var best = 0.0;
        for (final e in entries) {
          final n = double.tryParse('${e[field] ?? 0}') ?? 0;
          if (n > best) best = n;
        }
        return best;
      }

      final reward = maxPositive('reward_amount');
      final maxReward = maxPositive('max_reward');
      final minSpendValues = entries
          .map((e) => double.tryParse('${e['min_spend'] ?? 0}') ?? 0)
          .where((n) => n > 0)
          .toList();

      if (reward > 0) campaign['reward_amount'] = reward;
      if (maxReward > 0) campaign['max_reward'] = maxReward;
      if (minSpendValues.isNotEmpty) {
        campaign['min_spend'] = minSpendValues.reduce(
          (a, b) => a < b ? a : b,
        );
      }

      // Birden fazla banka/kart kuralı varsa ana kart alanlarını
      // birleştirerek detay ekranında da veri kaybolmasını engelle.
      String uniqueValues(String field) {
        final values = <String>{};
        for (final e in entries) {
          final v = '${e[field] ?? ''}'.trim();
          if (v.isNotEmpty && v.toLowerCase() != 'null') values.add(v);
        }
        return values.join(' / ');
      }

      final banks = uniqueValues('bank_name');
      final cards = uniqueValues('card_name');
      final networks = uniqueValues('network');

      if (banks.isNotEmpty) campaign['bank_name'] = banks;
      if (cards.isNotEmpty) campaign['card_name'] = cards;
      if (networks.isNotEmpty) campaign['network'] = networks;

      // UI'daki eski alan adlarını olduğu gibi koru; burada title/text
      // üzerinde herhangi bir yeniden yazım yapılmıyor.
      list.add(campaign);
    }
  ...
  bool cardMatches(
    ...
    if (rules is List && rules.isNotEmpty) {
      return rules.any(
        (r) => r is Map<String, dynamic>
            ? _singleRuleMatchesCard(r, userCard)
            : _singleRuleMatchesCard(
    ...
    return _singleRuleMatchesCard(campaign, userCard);
  }
...
String campaignConditions(Map<String, dynamic> campaign) {
  ...
  final conditions = cleanCampaignHtml('${campaign['conditions'] ?? ''}');
  final description = cleanCampaignHtml('${campaign['description'] ?? ''}');

  if (terms.length >= 40) return terms;
  if (conditions.length >= 40) return conditions;
  if (campaignText.length >= 40) return campaignText;
  if (description.length >= 40) return description;

  return [terms, conditions, campaignText, description]
      .where((x) => x.isNotEmpty)
      .join('\n\n');
}
