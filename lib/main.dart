  ...
  Future<void> loadCards() async {
    ...
    setState(() {
      ...
      cards.addAll(List<Map<String,dynamic>>.from(rows).map((m) => UserCard(
        id: m['id'].toString(), bank: m['bank_name'],
        card: m['card_name'], network: m['network'])));
    });
  }
...
class _CampaignsPageState extends State<CampaignsPage> {
  ...
  final quick = const [
    ['⛽', 'Akaryakıt'], ['🛒', 'Market'], ['🍔', 'Restoran'],
    ['🛍️', 'E-ticaret'], ['📱', 'Elektronik'], ['👕', 'Giyim'],
  ];
  ...
  Future<List<Map<String,dynamic>>> fetch() async {
    final r = await Supabase.instance.client.from('active_campaigns')
  ...
  bool matches(Map<String,dynamic> c) {
    ...
    final blob = c.values.map((v) => '$v').join(' ').toLowerCase();
    final qOk = q.isEmpty || blob.contains(q);
    final catOk = selectedCategory.isEmpty ||
        blob.contains(selectedCategory.toLowerCase());
    return qOk && catOk;
  }
    ...
    body: RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          ...
          const Text(
            'Ne alacağını yaz, sana uygun kart kampanyalarını bulalım.',
          ...
          Wrap(
            spacing: 8,
            runSpacing: 8,
            children: quick.map((x) => FilterChip(
              avatar: Text(x[0]),
              label: Text(x[1]),
              selected: selectedCategory == x[1],
              onSelected: (_) => setState(() {
                selectedCategory = selectedCategory == x[1] ? '' : x[1];
              }),
            )).toList(),
          ),
          ...
          Row(
            children: [
              const Expanded(
                child: Text('Sana uygun kampanyalar',
                    style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
              ),
              IconButton(
                onPressed: () => setState(() => future = fetch()),
          ...
          FutureBuilder<List<Map<String,dynamic>>>(
            ...
            builder: (_, s) {
              ...
              if (rows.isEmpty) return const Padding(
                ...
                child: Center(child: Text('Uygun aktif kampanya bulunamadı.')),
              );
              return Column(
                children: rows.map((c) => CampaignCard(campaign: c)).toList(),
              );
            },
          ),
        ],
...
class CampaignCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final amount = campaign['reward_amount'];
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (merchant.isNotEmpty) Padding(
            padding: const EdgeInsets.only(top: 7),
            child: Text(merchant),
          ),
          if (amount != null) Padding(
            padding: const EdgeInsets.only(top: 7),
            child: Text('Avantaj: $amount',
              style: const TextStyle(fontWeight: FontWeight.bold)),
          ),
          Padding(
            padding: const EdgeInsets.only(top: 7),
            child: Text('Son gün: $end'),
          ),
        ]),
      ),
    );
...
class _MyCardsPageState extends State<MyCardsPage> {
      ...
      builder: (ctx, setDialog) => AlertDialog(
        ...
        content: Column(mainAxisSize: MainAxisSize.min, children: [
          DropdownButtonFormField<String>(
            value: bank, decoration: const InputDecoration(labelText: 'Banka'),
          ...
          DropdownButtonFormField<String>(
            value: card, decoration: const InputDecoration(labelText: 'Kart'),
            items: (cardMap[bank]??[]).map((c)=>DropdownMenuItem(value:c,child:Text(c))).toList(),
            onChanged: (v){if(v!=null)setDialog(()=>card=v);}),
          DropdownButtonFormField<String>(
            value: network, decoration: const InputDecoration(labelText: 'Kart ağı'),
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    floatingActionButton: FloatingActionButton.extended(
      onPressed:addCard, icon:const Icon(Icons.add), label:const Text('Kart Ekle')),
    body: widget.cards.isEmpty
      ...
      : ListView(padding:const EdgeInsets.all(16),children:widget.cards.map((c)=>Card(
        child:ListTile(
