...
class _CampaignsPageState extends State<CampaignsPage> {
  ...
  bool _ruleMatches(String rule, String actual) {
    ...
    // Boş / yıldız / genel ifadeler kampanyanın herkese uygun olduğunu belirtir.
    if (r.isEmpty || r == '*' || r == 'all' || r == 'tum' || r == 'hepsi') {
      return true;
    }
    ...
    for (final item in rules) {
      if (item == a || a.contains(item) || item.contains(a)) {
        return true;
      }

      // Banka kartı / kredi kartı ve müşteri tipi için yaygın varyasyonlar.
      final normalizedItem = item
          .replaceAll('kart', '')
          .replaceAll('banka', '')
          .replaceAll('bankamatik', '')
          .replaceAll(RegExp(r'\s+'), ' ')
          .trim();

      final normalizedActual = a
          .replaceAll('kart', '')
          .replaceAll('banka', '')
          .replaceAll('bankamatik', '')
          .replaceAll(RegExp(r'\s+'), ' ')
      ...
      if (normalizedItem.isNotEmpty &&
          (normalizedItem == normalizedActual ||
              normalizedActual.contains(normalizedItem) ||
              normalizedItem.contains(normalizedActual))) {
        return true;
      }
    }
    return false;
