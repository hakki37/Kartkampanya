...
class _CampaignsPageState extends State<CampaignsPage> {
  ...
  Future<List<Map<String, dynamic>>> fetchCampaigns() async {
    // ÖNEMLİ:
    // campaign_rules tablosu kampanyanın kendisini ve kart kısıtlarını
    // birlikte içeriyor. Bu yüzden campaigns ile id üzerinden JOIN
    // yapılmamalı. Önceki sürümde bu yanlış eşleştirme nedeniyle
    // kampanyanın uygun kartlarında tüm kayıtlı kartlar görünebiliyordu.
    final rows = await Supabase.instance.client
        .from('campaign_rules')
        .select('*')
    ...
    for (final campaign in list) {
      campaign['title'] =
          ...
          campaign['campaign_title']?.toString() ??
          '';
      ...
      // eligible_networks JSON/text olarak tutuluyorsa network alanı
      // boş olduğunda onu da eşleşmede kullan.
      final network = '${campaign['network'] ?? ''}'.trim();
      if (network.isEmpty) {
        final eligible = campaign['eligible_networks'];
        if (eligible is List) {
          campaign['network'] = eligible.join('/');
        } else if (eligible != null &&
            '$eligible'.trim().isNotEmpty &&
            '$eligible' != 'null') {
          campaign['network'] = '$eligible'
              .replaceAll('[', '')
              .replaceAll(']', '')
              .replaceAll('"', '')
              .replaceAll("'", '');
  ...
  bool _ruleMatches(String rule, String actual) {
    ...
    bool matchesOne(String item) {
      ...
      return xs.any((left) => ys.any((right) => left == right));
    }
  ...
  String campaignId(Map<String, dynamic> campaign) =>
      '${campaign['id'] ?? campaign['detail_url'] ?? campaign['source_url'] ?? campaign['merchant'] ?? ''}';
