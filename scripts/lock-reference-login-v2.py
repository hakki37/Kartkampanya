...
start = s.index('class _LoginPageState extends State<LoginPage> {')
end = s.index('class MainShell extends StatefulWidget {', start)
login = s[start:end]

if 'Future<void> signInWithGoogle() async {' not in login:
    ...
    login = login.replace(anchor, replacement, 1)

if "'Google ile Giriş Yap'" not in login:
    anchor = '''                TextButton(\n                  onPressed: busy\n'''
    ...
    login = login.replace(anchor, button, 1)

if 'void dispose() {' not in login:
    anchor = '''\n\n  @override\n  Widget build(BuildContext context) {'''
    replacement = '''\n\n  @override\n  void dispose() {\n    email.dispose();\n    password.dispose();\n    super.dispose();\n  }\n\n  @override\n  Widget build(BuildContext context) {'''
    if anchor not in login:
        raise SystemExit('Login build anchor not found')
    login = login.replace(anchor, replacement, 1)

s = s[:start] + login + s[end:]
p.write_text(s, encoding='utf-8')
print('Google OAuth login implementation locked with scoped cleanup: provider, exact redirect URI, external browser launch, functional button, and controller disposal.')
