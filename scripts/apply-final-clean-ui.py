...
card = r'''class SmartCampaignCard extends StatelessWidget {
  ...
  String _clean(dynamic value) => value == null ? '' : '$value'
      .replaceAll(RegExp(r'<[^>]*>'), ' ')
      .replaceAll('&nbsp;', ' ')
      .replaceAll('&amp;', '&')
      .replaceAll('&quot;', '"')
  ...
  String _first(List<String> keys) {
    for (final key in keys) {
      final value = _clean(campaign[key]);
      if (value.isNotEmpty) return value;
    }
    return '';
  }

  String _logo() => _first(['logo_url', 'image_url', 'merchant_logo', 'brand_logo']);
  ...
  Widget build(BuildContext context) {
    ...
    final category = _first(['category']).isEmpty ? campaignSection(campaign) : _first(['category']);
        ...
        child: Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Container(
                width: 68, height: 68,
                decoration: BoxDecoration(color: const Color(0xFFF4F1FA), borderRadius: BorderRadius.circular(15)),
                child: logo.isNotEmpty ? ClipRRect(
                  borderRadius: BorderRadius.circular(15),
                  child: Image.network(logo, fit: BoxFit.contain, errorBuilder: (_, __, ___) => const Icon(Icons.local_offer_rounded, color: Color(0xFF6D3DF5), size: 30)),
                ) : const Icon(Icons.local_offer_rounded, color: Color(0xFF6D3DF5), size: 30),
              ),
              const SizedBox(width: 11),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    if (merchant.isNotEmpty) Text(merchant, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 10, color: Color(0xFF777187), fontWeight: FontWeight.w800)),
                    const SizedBox(height: 2),
                    Text(title.isEmpty ? 'Kampanya' : title, maxLines: 3, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 17, height: 1.12, color: Color(0xFF17142A), fontWeight: FontWeight.w900)),
                  ])),
                  IconButton(padding: EdgeInsets.zero, constraints: const BoxConstraints(minWidth: 34, minHeight: 34), onPressed: onFavorite, icon: Icon(isFavorite ? Icons.favorite_rounded : Icons.favorite_border_rounded, color: isFavorite ? const Color(0xFFE33F77) : const Color(0xFF666071), size: 27)),
                ]),
                if (expiringSoon) ...[
                  const SizedBox(height: 5),
                  Container(padding: const EdgeInsets.symmetric(horizontal: 9, vertical: 5), decoration: BoxDecoration(color: daysRemaining <= 3 ? const Color(0xFFFFE4E7) : const Color(0xFFE0F8E8), borderRadius: BorderRadius.circular(12)), child: Text('◷  Son $daysRemaining Gün', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w900, color: daysRemaining <= 3 ? const Color(0xFFD33A3A) : const Color(0xFF19864A))),
                ],
              ])),
            ]),
            if (description.isNotEmpty) ...[
              const SizedBox(height: 10),
              Text(description, maxLines: 3, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 13, height: 1.25, color: Color(0xFF5F5968))),
            ],
            if (benefit.isNotEmpty || minimum.isNotEmpty) ...[
              const SizedBox(height: 10),
              Row(children: [
                if (benefit.isNotEmpty) Expanded(child: Text('Tahmini avantaj: $benefit', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w900, color: Color(0xFF6D3DF5)))),
                if (minimum.isNotEmpty) Expanded(child: Text('Minimum harcama: $minimum', textAlign: TextAlign.right, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w800, color: Color(0xFF5F5968)))),
              ]),
            ],
            const SizedBox(height: 10),
            ...
            SizedBox(width: double.infinity, height: 43, child: FilledButton(
  ...
  Widget _tag(IconData icon, String label) {
    ...
    return Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 7), decoration: BoxDecoration(color: const Color(0xFFF0EDFA), borderRadius: BorderRadius.circular(11)), child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(icon, size: 14, color: const Color(0xFF332B50)), const SizedBox(width: 5), ConstrainedBox(constraints: const BoxConstraints(maxWidth: 130), child: Text(value, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 10, color: Color(0xFF332B50), fontWeight: FontWeight.w800)))]));
  }
}
'''
s = replace_class(s, 'SmartCampaignCard', card)
s = s.replace('Color(0xFF081120)', 'Color(0xFFF8F7FC)').replace('Color(0xFF0B1B31)', 'Colors.white')
p.write_text(s, encoding='utf-8')
print('Campaign card semantics and light catalog UI applied')