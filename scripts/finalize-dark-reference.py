...
# Force the catalog theme itself to dark. Login remains light because its
# screen has its own explicit background/colors.
...
card = r'''class SmartCampaignCard extends StatelessWidget {
  ...
  String text(dynamic value) => value == null ? '' : decodeHtmlEntities('$value').replaceAll(RegExp(r'<[^>]*>'), ' ').replaceAll(RegExp(r'\s+'), ' ').trim();
  ...
  String merchantDomain(String value) {
    ...
    const domains = <String, String>{
      'migros': 'migros.com.tr', 'carrefour': 'carrefoursa.com',
      'trendyol': 'trendyol.com', 'hepsiburada': 'hepsiburada.com',
      'amazon': 'amazon.com.tr', 'a101': 'a101.com.tr', 'bim': 'bim.com.tr',
      'sok': 'sokmarket.com.tr', 'opet': 'opet.com.tr', 'shell': 'shell.com.tr',
      'petrol ofisi': 'petrolofisi.com.tr', 'mediamarkt': 'mediamarkt.com.tr',
      'teknosa': 'teknosa.com', 'boyner': 'boyner.com.tr', 'mavi': 'mavi.com',
      'ikea': 'ikea.com.tr', 'ispark': 'ispark.istanbul',
    };
  ...
  String bankDomain(String value) {
    final m = _norm(value);
    const domains = <String, String>{
      'akbank': 'akbank.com', 'axess': 'axess.com.tr', 'garanti': 'garantibbva.com.tr',
      'bonus': 'bonus.com.tr', 'vakifbank': 'vakifbank.com.tr', 'world': 'worldcard.com.tr',
      'yapi kredi': 'worldcard.com.tr', 'qnb': 'qnb.com.tr', 'cardfinans': 'cardfinans.com.tr',
      'teb': 'teb.com.tr', 'cepteteb': 'teb.com.tr', 'hsbc': 'hsbc.com',
      'isbank': 'isbank.com.tr', 'maximum': 'maximum.com.tr', 'ziraat': 'ziraatbank.com.tr',
      'denizbank': 'denizbank.com', 'enpara': 'enpara.com', 'kuveytturk': 'kuveytturk.com.tr',
      'ing': 'ing.com.tr',
    };
    for (final e in domains.entries) {
      if (m.contains(e.key)) return e.value;
  ...
  Widget build(BuildContext context) {
    ...
    for (final c in cards) {
      for (final v in [c.bank, c.card, c.network]) {
        if (v.trim().isNotEmpty && !labels.contains(v.trim())) labels.add(v.trim());
      }
    }
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(22),
            border: Border.all(color: const Color(0xFF243A57), width: 1.1),
          ),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Container(
                ...
                child: merchantDomain(merchant).isNotEmpty
                    ? Image.network(logo(merchantDomain(merchant)), fit: BoxFit.contain, filterQuality: FilterQuality.high, errorBuilder: (_, __, ___) => Text(merchant.isEmpty ? 'KAMPANYA' : merchant, maxLines: 2, overflow: TextOverflow.ellipsis, textAlign: TextAlign.center, style: const TextStyle(color: Color(0xFF172033), fontWeight: FontWeight.w900, fontSize: 11)))
                    : Text(merchant.isEmpty ? 'KAMPANYA' : merchant, maxLines: 2, overflow: TextOverflow.ellipsis, textAlign: TextAlign.center, style: const TextStyle(color: Color(0xFF172033), fontWeight: FontWeight.w900, fontSize: 11)),
              ),
              const SizedBox(width: 11),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(title.isEmpty ? 'Kampanya' : title, maxLines: 3, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.white, fontSize: 16, height: 1.15, fontWeight: FontWeight.w900)),
              ...
              Column(children: [
                IconButton(visualDensity: VisualDensity.compact, padding: EdgeInsets.zero, constraints: const BoxConstraints(minWidth: 32, minHeight: 32), onPressed: onFavorite, icon: Icon(isFavorite ? Icons.star_rounded : Icons.star_border_rounded, color: isFavorite ? const Color(0xFFC5B5FF) : const Color(0xFF7E8CA3), size: 28)),
                IconButton(visualDensity: VisualDensity.compact, padding: EdgeInsets.zero, constraints: const BoxConstraints(minWidth: 32, minHeight: 32), onPressed: onCompare, icon: Icon(Icons.compare_arrows_rounded, color: isCompared ? const Color(0xFFC5B5FF) : const Color(0xFF7E8CA3), size: 25)),
              ]),
            ]),
            if (labels.isNotEmpty) ...[const SizedBox(height: 11), SizedBox(height: 40, child: ListView.separated(scrollDirection: Axis.horizontal, itemCount: labels.length > 4 ? 4 : labels.length, separatorBuilder: (_, __) => const SizedBox(width: 7), itemBuilder: (_, i) { final d = bankDomain(labels[i]); return Container(width: 82, padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 6), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(9)), alignment: Alignment.center, child: d.isEmpty ? Text(labels[i], maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Color(0xFF172033), fontSize: 10, fontWeight: FontWeight.w900)) : Image.network(logo(d), fit: BoxFit.contain, errorBuilder: (_, __, ___) => Text(labels[i], maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Color(0xFF172033), fontSize: 10, fontWeight: FontWeight.w900))); })),],
            ...
            Row(children: [
              Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6), decoration: BoxDecoration(color: const Color(0xFF172943), borderRadius: BorderRadius.circular(15), border: Border.all(color: const Color(0xFF2A4668))), child: Text(category, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Color(0xFFD6CCFF), fontSize: 11, fontWeight: FontWeight.w800))),
              const SizedBox(width: 8),
              Expanded(child: Text(cards.isNotEmpty ? 'Kayıtlı kartlarınla eşleşiyor' : 'Kayıtlı kartlarınla eşleşmiyor', maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Color(0xFF7F8DA4), fontSize: 10.5, fontWeight: FontWeight.w600))),
              const Icon(Icons.chevron_right_rounded, color: Color(0xFF93A1B6), size: 23),
            ]),
          ]),
        ),
      ),
        ...
        builder: (context, snapshot) {
          ...
          return RefreshIndicator(
            ...
            child: ListView(padding: const EdgeInsets.fromLTRB(16, 14, 16, 90), children: [
              Row(children: [
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  const Text('Kart Kampanya', style: TextStyle(color: Colors.white, fontSize: 25, fontWeight: FontWeight.w900)),
                  const SizedBox(height: 3),
                  const Text('Tüm banka kampanyaları tek yerde!', style: TextStyle(color: Color(0xFFAAB6C8), fontSize: 12.5, fontWeight: FontWeight.w500)),
                ])),
                IconButton(onPressed: () => setState(() => future = fetchCampaigns()), icon: const Icon(Icons.refresh_rounded, color: Colors.white, size: 27)),
                IconButton(onPressed: () => Supabase.instance.client.auth.signOut(), icon: const Icon(Icons.logout_rounded, color: Colors.white, size: 26)),
              ]),
              ...
              GridView.builder(shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), itemCount: quickItems.length, gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 4, crossAxisSpacing: 9, mainAxisSpacing: 9, childAspectRatio: .98), itemBuilder: (_, i) {
                final item = quickItems[i];
                final selected = category == item[1];
                return InkWell(onTap: () => setState(() => category = selected ? '' : item[1]), borderRadius: BorderRadius.circular(18), child: Container(decoration: BoxDecoration(color: selected ? const Color(0xFF6245EF) : const Color(0xFF0E1B30), borderRadius: BorderRadius.circular(18), border: Border.all(color: selected ? const Color(0xFF866FFF) : const Color(0xFF294664))), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Text(item[0], style: const TextStyle(fontSize: 25)), const SizedBox(height: 5), Text(item[1], maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.white, fontSize: 11.5, fontWeight: FontWeight.w800))])));
              }),
              const SizedBox(height: 14),
              SingleChildScrollView(scrollDirection: Axis.horizontal, child: Row(children: [
                _catalogFilter('Tümü', !showFavoritesOnly && category.isEmpty, () => setState(() { category = ''; showFavoritesOnly = false; })),
                _catalogFilter('✦  Bana Uygun', widget.mode == 'matched', () => setState(() { category = ''; showFavoritesOnly = false; })),
                _catalogFilter('★  Favoriler', showFavoritesOnly, () => setState(() => showFavoritesOnly = !showFavoritesOnly)),
                _catalogFilter('Son Eklenen', false, () => setState(() => showAllCampaigns = true)),
              ])),
...
# Prevent an accidental old light scaffold from surviving in catalog-only pages.
# Explicit login colors are intentionally outside CampaignsPage and are untouched.
...
print('Final dark catalog reference locked')
